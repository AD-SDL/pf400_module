...
def check_state(state: State):
    ...
    if state.pf400.robot_warning.upper() != "CLEAR" and len(state.pf400.robot_warning) > 0:
        state.status = ModuleStatus.ERROR
    ...
    elif (movement_state >= 1 and state.status == ModuleStatus.BUSY) or movement_state >= 2:
...
def remove_lid(state: State, action: ActionRequest,
