...
from pf400_driver.tcp_driver import PF400
...
class PF400_CAMERA:
    def __init__(self, robot_connection):
        ...
        # Store joint angles
        # Make sure linear axis length is removed from the x axis
    ...
    def explore_workcell(self):
        ...
        for i in range(4):
            ...
            if (
                self.cam_left_qr_name not in self.module_list.values()
                and self.cam_left_qr_name in self.locations.keys()
            ):
                self.locations[self.cam_left_qr_name][5] = self.start_location[i]
                self.module_list[
                    i + 1
                ] = self.cam_left_qr_name  # Add the module into module list
                print(self.locations[self.cam_left_qr_name])

            if (
                self.cam_right_qr_name not in self.module_list.values()
                and self.cam_right_qr_name in self.locations.keys()
            ):
                # TODO:Change this to a function (def reverse_module_location)

                cartesian, phi, rail = self.pf400.forward_kinematics(
                    self.locations[self.cam_right_qr_name]
                )
                if self.start_location == -990:
                    ...
                    reverse_target_on_x_axis = (
                        self.module_length - target_on_x_without_rail
                    )

                    cartesian[0] = reverse_target_on_x_axis + self.start_location[i]
                    cartesian[1] = -cartesian[
                        1
                    ]  # Switch arm from left to right on y axis
                    cartesian[3] -= 180
                    self.locations[
                        self.cam_right_qr_name
                    ] = self.pf400.inverse_kinematics(
                        cartesian_coordinates=cartesian,
                        phi=phi,
                        rail=self.start_location[i],
                    )
                else:
                    target_on_x_without_rail = cartesian[0] - rail
                    reverse_target_on_x_axis = (
                    ...
                    rail_travel = (
                        reverse_target_on_x_axis - target_on_x_without_rail
                    )  # Find the length in between new target location and old target location
                    ...
                    cartesian[0] = (
                        target_on_x_without_rail + total_rail_travel
                    )  # Keeping the same x axis value while setting a new value to rail to move to the new location.
                    cartesian[1] = -cartesian[
                        1
                    ]  # Switch arm from left to right on y axis
                    cartesian[3] -= 180
                    self.locations[
                        ...
                        cartesian_coordinates=cartesian, phi=phi, rail=total_rail_travel
                    )

                self.locations[self.cam_right_qr_name][5] = self.start_location[i]
                self.module_list[
                    i + 5
                ] = self.cam_right_qr_name  # Add the module into module list
    ...
    def calctulate_module_location(
        self, target_loc, y_direction=1, reverse_x=0.0, offset_y=0.0
    ):
        pass
