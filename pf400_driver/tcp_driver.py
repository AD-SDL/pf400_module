...
file_path = os.path.join(
    os.path.split(os.path.dirname(__file__))[0] + "/pf400_logs/robot_client_logs.log"
)
...
class PF400:
    ...
    def __init__(
        self,
        data_file_path="robot_data.json",
        commands_file_path="robot_commands.json",
        error_codes_path="error_codes.json",
    ):
        ...
        # Predefined locations for plate transferring operations
        ...
        self.logger.info(
            "Robot created. Robot ID: {} ~ Host: {} ~ Port: {}".format(
    ...
    def load_robot_data(self, data_file_path):
        """
        Description: Loads the robot identification/motion profile/location data as dictionaries
        Parameters:
        ...
        return (
            data,
            data["robot_data"][0],
            data["robot_data"][0]["motion_profile"],
            data["robot_data"][0]["locations"][0],
        )

    def load_robot_commands(self, commands_file_path):
        """
        Description: Loads the available command list as a list
        Parameters:
    ...
    def load_error_codes(self, error_codes_path):
        """
        Description: Loads the robot error codes data as a dictionary
        Parameters:
    ...
    def connect_robot(self):
        """
        Description: Create an INET, Streaming socket (IPv4, TCP/IP) to send string commands to the robot.
    ...
    def send_command(
        self,
        cmd: str = None,
        ini_msg: str = "Send command",
        err_msg: str = "Failed to send command: ",
        wait: int = 0.1,
    ):
        """
        Description: Sends the commands to the robot over the TCP socket
        Parameters:
        ...
        try:
            ...
            # If command was unsuccessful, print the related error message
    ...
    def set_profile(self, wait: int = 0.1, profile_dict: dict = {"0": 0}):
        ...
        if len(profile_dict) == 1:
            ...
            out_msg = self.send_command(cmd, ini_msg, err_msg, wait)
            self.send_command(cmd2, ini_msg2, err_msg2, wait)
    ...
    def initialize_robot(self):
        """
        Description: Intilizes the robot by calling enable_power, attach_robot, home_robot, set_profile functions and
    ...
    def force_initialize_robot(self):
        """
        Description: Repeats the initilzation until there are no errors and the robot is initilzed.
        """
    ...
    def set_robot_mode(self):
        """
        Description: Sets the robot to PC mode. This is needed to make sure the robot is properly communicating over the TCP socket.
        """
    ...
    def check_robot_heartbeat(self, wait: int = 0.1):
        """
        Description: Checks the robot heartbeat.
        """
        cmd = "nop\n"
    ...
    def stop_robot(self, wait: int = 0.1):
        """
        Description: Stops the robot immediately but leaves power on.
        """
    ...
    def wait_before_next_move(self, wait: int = 0.1):
        """
        Description: **NOT IMPLEMENTED** If you want to wait for the robot to stop moving, issue a waitForEom command
        """
        pass

    def clear_programs(self, wait: int = 0.1):
        """
        Description: TODO: CLEAR ROBOT MEMORY BEFORE STARTING A PROGRAM TO MAKE SURE THERE IS NO QUEUED PROGRAMS FROM PREVIOUS EXECUTION
        """
        pass

    def set_move_command(
        self,
        target_location,
        profile: int = 2,
        ...
        if profile == 1:
            ...
            raise Exception(
                "Please enter a valid motion profiile! 1 for slower movement, 2 for faster movement profile, 3 for modified profile"
            )

        for count, location in enumerate(self.location_dictionary[target_location]):
            if gripper_close and count == 4:
                robot_command += " " + str(120.0)
            elif gripper_open and count == 4:
    ...
    def move_single(
        ...
        """
        Description: Executes only one movement to the target location which is from the location dictionary
        Parameters:
                - target_location: Which location the PF400 will move.
                - profile: Motion profile ID.
                - gripper_close: If set to TRUE, gripper is closed. If set to FALSE, gripper position will remain same as the previous location.
        ...
        """
        single_move_commands = self.set_move_command(
            target_location.lower(), profile, gripper_close, gripper_open
        )
    ...
    def manualy_move_cartesian(self, target_joint):
        """
        Description: ** NOT IMPLEMENTED ** A Cartesian location specifies the coordinates of a position in space using X, Y, and Z coordinates, and an orientation of the robot tool using yaw, pitch, and roll angles.
    ...
    def manualy_move_joints(self, target_joint_location):
        """
        Description: ** NOT IMPLEMENTED ** An angles location is a collection of the joint angles for the robot.
    ...
    def set_speed(self, speed: str = 50, wait: int = 0.1):
        """
        Description: Set the robot speed.
        Parameters:
    ...
    def check_loc_data(self, target_location):
        """
        Description: Checks if the given location exists in the location data.
        ...
        if idx >= 0:
            ...
            self.logger.error("Target location doesn't exist in the robot data file")
            return False

    def teach_new_location(self, location_name: str, gripper: str = 127):
        """
        Description: ** NOT IMPLEMENTED ** A function to save a new location data into the location data file with a given name by the user.
        """
